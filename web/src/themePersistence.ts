...
/**
 * The theme to paint first. Synchronous, so main.tsx can apply it before the
 * first render and no frame ever shows the wrong theme (Requirement 5.3).
 *
 * An unknown id or unreadable storage (a private window can throw on access)
 * yields the default silently, and nothing is written back: a stored theme
 * that is only temporarily unregistered must survive (Requirement 5.4).
 */
export function readStoredTheme(): Theme {
  try {
    return findTheme(localStorage.getItem(THEME_STORAGE_KEY)) ?? getTheme(DEFAULT_THEME_ID);
  } catch {
