...
- **Don't trust the `push`-to-`main` trigger alone.** Both `deploy-backend.yml` and `deploy-web.yml` fire on `push: branches: [main]` with a `paths` filter, and both also support `workflow_dispatch`. On 2026-08-06, merges to `main` repeatedly failed to auto-trigger the matching deploy workflow — the merge lands, CI shows nothing, and the live Azure app keeps serving the old build with zero error signal anywhere. Root cause that day was confirmed external: a GitHub-wide Actions incident (see githubstatus.com — "webhook triggers throttled, processing ~15% of webhooks," only ~65% of queued jobs succeeding, incident window 2026-08-06 15:22–20:34+ UTC), not a repo-side misconfiguration — check status.github.com first if this recurs, since a live incident means retries/dispatches will also be slow or stuck (queued with no runner assigned) until GitHub recovers, not something to keep hammering. Regardless of cause, the standing practice: **after every merge that touches `backend/**` or `web/**`, verify the matching workflow actually queued a run for that commit's SHA** (`GET /actions/workflows/{id}/runs`) — if not, fire it yourself: `POST /actions/workflows/{id}/dispatches` with `{"ref":"main"}`. Then confirm the live site reflects it (e.g. diff the served JS/CSS bundle hash, or hit `/health`) — the Kudu redeploy hook it calls is itself async (returns an `OperationId` immediately, container swap happens after), so allow a minute or two before concluding a dispatch didn't take effect.
