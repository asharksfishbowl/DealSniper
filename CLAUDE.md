...
- **Don't trust the `push`-to-`main` trigger alone.** Both `deploy-backend.yml` and `deploy-web.yml` fire on `push: branches: [main]` with a `paths` filter, and both also support `workflow_dispatch`. In practice (observed repeatedly on 2026-08-06) merges to `main` have silently failed to auto-trigger the matching deploy workflow — the merge lands, CI shows nothing, and the live Azure app keeps serving the old build with zero error signal anywhere. No cause was found (repo has no registered webhooks per `/repos/.../hooks`, so it's not a broken third-party hook; GitHub Actions' own push-trigger evaluation has no exposed delivery log to check). **After every merge that touches `backend/**` or `web/**`, verify the matching workflow actually queued a run for that commit's SHA** (`GET /actions/workflows/{id}/runs`) — if not, fire it yourself: `POST /actions/workflows/{id}/dispatches` with `{"ref":"main"}`. Then confirm the live site reflects it (e.g. diff the served JS/CSS bundle hash, or hit `/health`) — the Kudu redeploy hook it calls is itself async (returns an `OperationId` immediately, container swap happens after), so allow a minute or two before concluding a dispatch didn't take effect.
